"""Cloud Tasks enqueue for approved Meridian fits. Payload is dispatch_id only."""
...
from app.modeling.mmm.contracts import MeridianFitDispatch
from app.service.evaluation_dispatch import DispatchEnqueueError, _already_exists

try:
    from google.cloud import tasks_v2
except ImportError:  # pragma: no cover - local tests use FakeFitDispatcher
    tasks_v2 = None


class FitDispatcher(Protocol):
    def enqueue(self, dispatch: MeridianFitDispatch) -> str: ...


class FakeFitDispatcher:
    ...
    def enqueue(self, dispatch: MeridianFitDispatch) -> str:
        if self.fail_next:
            self.fail_next = False
            raise DispatchEnqueueError("fake fit enqueue failed")
        task_name = f"prem3-mmm-{dispatch.dispatch_id}"
        self.calls.append(
            {
                "task_name": task_name,
...
class UnavailableFitDispatcher:
    def enqueue(self, dispatch: MeridianFitDispatch) -> str:
        del dispatch
        raise DispatchEnqueueError("durable fit dispatcher is not configured")


class CloudTasksFitDispatcher:
    def __init__(
        self,
        *,
        project_id: str,
    ...
    def enqueue(self, dispatch: MeridianFitDispatch) -> str:
        task_id = f"prem3-mmm-{dispatch.dispatch_id}"
        ...
        try:
            ...
            if _already_exists(exc):
                return task_name
            raise DispatchEnqueueError("cloud tasks fit enqueue failed") from exc
